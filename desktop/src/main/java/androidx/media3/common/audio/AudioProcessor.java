 ...
 *
 * Desktop port (Stash macOS): media3 1.9.2 source, unchanged except that the
 * AudioFormat(Format) constructor is omitted (no Format class on desktop) and the
 * Format/Util helpers it used are inlined as private members of AudioFormat.
 */
...
public interface AudioProcessor {
  ...
  final class AudioFormat {
    ...
    // Desktop port: media3's Format.NO_VALUE (this file does not pull in Format).
    ...
    public AudioFormat(int sampleRate, int channelCount, @C.PcmEncoding int encoding) {
      ...
      bytesPerFrame =
          isEncodingLinearPcm(encoding)
              ? getPcmFrameSize(encoding, channelCount)
              : NO_VALUE;
    }
    ...
    // Desktop port: the two Util PCM helpers used above, copied verbatim so this file does not
    // pull in Util.

    private static boolean isEncodingLinearPcm(@C.Encoding int encoding) {
    ...
    private static int getPcmFrameSize(@C.PcmEncoding int pcmEncoding, int channelCount) {
      int byteDepth;
      switch (pcmEncoding) {
